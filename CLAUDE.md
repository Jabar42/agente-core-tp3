...
Monorepo del agente de chat multi-cliente de Tp3studio. Worker Cloudflare con Agents SDK + Cloudflare AI Gateway → DeepSeek, widget React publicable a npm.
...
```
packages/
├── agent/              ← Worker Cloudflare (Agents SDK + AI Gateway → DeepSeek)
│   ├── src/index.ts           ← Tp3ChatAgent + fetch handler (llamadas via AI Gateway /compat)
│   ├── src/prompts.ts         ← Mapa PROMPTS por cliente
│   ├── src/dashboard/         ← Panel admin (métricas, conversaciones, prompts)
...
4. **Setear token del AI Gateway**:
   ```bash
   cd packages/agent
   echo "<gateway-token>" | npx wrangler -c wrangler-<cliente>.jsonc secret put AI_GATEWAY_TOKEN
   ```
   El token se obtiene del dashboard de Cloudflare AI Gateway (el mismo para todos los clientes).
